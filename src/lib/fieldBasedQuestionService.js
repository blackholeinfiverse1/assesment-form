...
class FieldBasedQuestionService {
  ...
  async getQuestionsFromDatabase(
    ...
    try {
      ...
      // Ultimate fallback: drop category filter entirely to avoid empty results
      if (pool.length < count) {
        console.log(
          `🛑 Ultimate fallback: dropping category filter and returning any available questions`
        );
        ...
        if (excludeAI) {
          anyQuery = anyQuery.eq("created_by", "admin");
        }

        // Prefer matching difficulty if provided
        if (difficulty) {
          anyQuery = anyQuery.ilike("difficulty", difficulty);
        }

        // Only apply tags when not in admin-only mode
        if (!excludeAI && Array.isArray(levelTags) && levelTags.length > 0) {
          anyQuery = anyQuery.overlaps("tags", levelTags);
        }

        const { data: anyData, error: anyErr } = await anyQuery;
        if (!anyErr && anyData) {
          console.log(
            `✅ Any-category fallback returned ${anyData.length} questions`
          );
          const ids3 = new Set(pool.map((q) => q.question_id));
          anyData.forEach((q) => {
            if (!ids3.has(q.question_id)) pool.push(q);
          });
        } else if (anyErr) {
          console.error("❌ Any-category query error:", anyErr);
        }
      }

      // If still not enough and admin-only filter was applied, unlock AI-authored questions
      if (pool.length < count && excludeAI) {
        console.log(
          `🧩 Fallback extension: including AI-authored questions as none found for admin-only`
        );
        let anyWithAIQuery = supabase
          .from("question_banks")
          .select("*")
          .eq("is_active", true)
          .limit(count * 3);

        // Try to keep difficulty if available
        if (difficulty) {
          anyWithAIQuery = anyWithAIQuery.ilike("difficulty", difficulty);
        }

        // Apply tags if provided
        if (Array.isArray(levelTags) && levelTags.length > 0) {
          anyWithAIQuery = anyWithAIQuery.overlaps("tags", levelTags);
        }

        const { data: anyWithAIData, error: anyWithAIErr } = await anyWithAIQuery;
        if (!anyWithAIErr && anyWithAIData) {
          console.log(
            `✅ Any-category (including AI) returned ${anyWithAIData.length} questions`
          );
          const ids4 = new Set(pool.map((q) => q.question_id));
          anyWithAIData.forEach((q) => {
            if (!ids4.has(q.question_id)) pool.push(q);
          });
        } else if (anyWithAIErr) {
          console.error("❌ Any-category (including AI) query error:", anyWithAIErr);
        }
      }

      // Last resort: drop difficulty as well
      if (pool.length < count) {
        console.log(`🪂 Last resort: dropping difficulty constraint too`);
        ...
        // Keep admin-only if requested
        if (excludeAI) {
          looseQuery = looseQuery.eq("created_by", "admin");
        }

        // Apply tags if not admin-only
        if (!excludeAI && Array.isArray(levelTags) && levelTags.length > 0) {
          looseQuery = looseQuery.overlaps("tags", levelTags);
        }

        const { data: looseData, error: looseErr } = await looseQuery;
        if (!looseErr && looseData) {
          console.log(`✅ Loose fallback returned ${looseData.length} questions`);
          const ids5 = new Set(pool.map((q) => q.question_id));
          looseData.forEach((q) => {
            if (!ids5.has(q.question_id)) pool.push(q);
          });
        } else if (looseErr) {
          console.error("❌ Loose fallback query error:", looseErr);
        }
      }
